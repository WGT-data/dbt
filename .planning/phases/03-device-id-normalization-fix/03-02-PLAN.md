...
- Grain: one row per DATE + PLATFORM + CHANNEL
  ...
  <verify>
Both files exist and have valid SQL. Run `dbt compile --select int_mmm__daily_channel_spend int_mmm__daily_channel_installs` to verify SQL compiles without errors. If dbt compile is not available locally, verify by reading the compiled SQL and checking that all ref() calls point to existing models and all column names match the source models.
  </verify>
  <done>
