...
--
-- SAN HANDLING: Self-Attributing Networks (Meta, Google) do not provide
-- SK_TRANSACTION_ID in their SKAN postbacks. We use NONCE as a fallback
-- unique postback identifier via COALESCE(SK_TRANSACTION_ID, NONCE).
--
...
WITH skan_installs AS (
    SELECT
        ...
        , SK_DID_WIN
        , SK_VERSION
        -- SANs (Meta, Google) don't provide SK_TRANSACTION_ID; use NONCE as fallback
        , COALESCE(SK_TRANSACTION_ID, NONCE) AS POSTBACK_ID
...
, aggregated AS (
    SELECT
        ...
        -- Install counts (POSTBACK_ID = SK_TRANSACTION_ID for non-SANs, NONCE for SANs)
        , COUNT(DISTINCT POSTBACK_ID) AS INSTALL_COUNT
        , COUNT(DISTINCT CASE WHEN SK_REDOWNLOAD = '0' THEN POSTBACK_ID END) AS NEW_INSTALL_COUNT
        , COUNT(DISTINCT CASE WHEN SK_REDOWNLOAD = '1' THEN POSTBACK_ID END) AS REDOWNLOAD_COUNT
        ...
        , COUNT(DISTINCT CASE WHEN SK_CONVERSION_VALUE IS NOT NULL THEN POSTBACK_ID END) AS INSTALLS_WITH_CV

        -- Conversion value distribution buckets
        , COUNT(DISTINCT CASE WHEN TRY_CAST(SK_CONVERSION_VALUE AS INTEGER) = 0 THEN POSTBACK_ID END) AS CV_BUCKET_0
        , COUNT(DISTINCT CASE WHEN TRY_CAST(SK_CONVERSION_VALUE AS INTEGER) BETWEEN 1 AND 10 THEN POSTBACK_ID END) AS CV_BUCKET_1_10
        , COUNT(DISTINCT CASE WHEN TRY_CAST(SK_CONVERSION_VALUE AS INTEGER) BETWEEN 11 AND 20 THEN POSTBACK_ID END) AS CV_BUCKET_11_20
        , COUNT(DISTINCT CASE WHEN TRY_CAST(SK_CONVERSION_VALUE AS INTEGER) BETWEEN 21 AND 40 THEN POSTBACK_ID END) AS CV_BUCKET_21_40
        , COUNT(DISTINCT CASE WHEN TRY_CAST(SK_CONVERSION_VALUE AS INTEGER) BETWEEN 41 AND 63 THEN POSTBACK_ID END) AS CV_BUCKET_41_63

        -- Fidelity type breakdown (1 = StoreKit rendered, 0 = view-through)
        , COUNT(DISTINCT CASE WHEN SK_FIDELITY_TYPE = '1' THEN POSTBACK_ID END) AS STOREKIT_RENDERED_COUNT
        , COUNT(DISTINCT CASE WHEN SK_FIDELITY_TYPE = '0' THEN POSTBACK_ID END) AS VIEW_THROUGH_COUNT

        -- Win rate (did this network win the attribution)
        , COUNT(DISTINCT CASE WHEN SK_DID_WIN = '1' THEN POSTBACK_ID END) AS WINNING_POSTBACKS

        -- SKAN version distribution
        , COUNT(DISTINCT CASE WHEN SK_VERSION LIKE '3%' THEN POSTBACK_ID END) AS SKAN_V3_COUNT
        , COUNT(DISTINCT CASE WHEN SK_VERSION LIKE '4%' THEN POSTBACK_ID END) AS SKAN_V4_COUNT
