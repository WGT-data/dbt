...
-- Aggregate by key to collapse any fan-out from the FULL OUTER JOIN
-- (case-insensitive matching can produce multiple rows per logical key)
SELECT
    INSTALL_DATE
    , AD_PARTNER
    , CAMPAIGN_NAME
    ...
    -- SKAN installs
    , SUM(SKAN_INSTALL_COUNT) AS SKAN_INSTALL_COUNT
    , SUM(SKAN_NEW_INSTALLS) AS SKAN_NEW_INSTALLS
    , SUM(SKAN_REDOWNLOADS) AS SKAN_REDOWNLOADS

    -- Efficiency metrics (from aggregated totals)
    , CASE WHEN SUM(SKAN_NEW_INSTALLS) > 0
        THEN SUM(COST) / SUM(SKAN_NEW_INSTALLS)
    ...
    , CASE WHEN SUM(CLICKS) > 0
        THEN SUM(SKAN_NEW_INSTALLS)::FLOAT / SUM(CLICKS)
        ELSE NULL END AS SKAN_CVR

    -- Win rate
    , CASE WHEN SUM(SKAN_INSTALL_COUNT) > 0
        THEN SUM(WINNING_POSTBACKS)::FLOAT / SUM(SKAN_INSTALL_COUNT)
        ELSE NULL END AS WIN_RATE

    -- Fidelity mix
    , CASE WHEN SUM(SKAN_INSTALL_COUNT) > 0
        THEN SUM(STOREKIT_RENDERED_COUNT)::FLOAT / SUM(SKAN_INSTALL_COUNT)
        ELSE NULL END AS STOREKIT_RENDERED_RATE
    , CASE WHEN SUM(SKAN_INSTALL_COUNT) > 0
        THEN SUM(VIEW_THROUGH_COUNT)::FLOAT / SUM(SKAN_INSTALL_COUNT)
        ELSE NULL END AS VIEW_THROUGH_RATE

    -- CV coverage
    , CASE WHEN SUM(SKAN_INSTALL_COUNT) > 0
        THEN SUM(INSTALLS_WITH_CV)::FLOAT / SUM(SKAN_INSTALL_COUNT)
        ELSE NULL END AS CV_COVERAGE_RATE

    -- Conversion value metrics
    , MAX(AVG_CONVERSION_VALUE) AS AVG_CONVERSION_VALUE
    , MAX(MAX_CONVERSION_VALUE) AS MAX_CONVERSION_VALUE
    , SUM(INSTALLS_WITH_CV) AS INSTALLS_WITH_CV
    , SUM(CV_BUCKET_0) AS CV_BUCKET_0
    , SUM(CV_BUCKET_1_10) AS CV_BUCKET_1_10
